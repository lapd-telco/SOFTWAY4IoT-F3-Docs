...
app.post('/commandresult', function (req, res) {
    ...
    var requestOptions = {
        headers: fiwareServiceHeaders,
        url: 'http://' + config.orion.host + ':' + config.orion.port + '/v2/entities/urn:ngsi-ld:Thing:' + config.devices.servo.id + '/attrs/' + config.devices.servo.attribute + '_info?type=Thing&options=keyValues',
        method: 'GET'
	};
...
app.post('/sendcommand', function (req, res) {
			...
			"contextElements": [
				{
					...
					"attributes": [
						{
							"name": config.devices.servo.attribute,
							"type": "command",
							"value": ""
						}
					]
	...
	axios(requestOptions)
		.then(function (response) {
			console.log(config.devices.servo.id + " " +  config.devices.servo.attribute);
			console.log(response.data.contextResponses[0]);
